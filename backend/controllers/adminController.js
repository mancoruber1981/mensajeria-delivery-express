...
const getDashboardStats = asyncHandler(async (req, res) => {
  try {
    ...
    const totalAPagarPromise = TimeLog.aggregate([
      { $lookup: { from: 'employees', localField: 'employee', foreignField: '_id', as: 'employeeInfo' } },
      { $unwind: '$employeeInfo' },
      { $match: { 'employeeInfo.employeeType': 'empresa' } },
      { $group: { _id: null, total: { $sum: '$valorNetoFinal' } } }
    ]);
...
const getAccountantLedger = asyncHandler(async (req, res) => {
  ...
  const incomePromise = TimeLog.aggregate([
    ...
    { $unwind: '$employeeInfo' },
    { $match: { 'employeeInfo.employeeType': 'cliente' } },
    { $group: { _id: '$empresa', totalAmount: { $sum: '$valorNetoFinal' }, firstDate: { $first: '$date' } } },
    { $lookup: { from: 'clients', localField: '_id', foreignField: 'companyName', as: 'clientInfo' } },
    { $unwind: '$clientInfo' },
    { $project: {
        _id:0, type:'income', date:'$firstDate', description:{ $concat:["Servicio a ","$_id"] },
        amount:'$totalAmount', cedula:'$clientInfo.nit', telefono:'$clientInfo.phone',
        direccion:'$clientInfo.address', email:'$clientInfo.email'
      }
    }
  ]);
  ...
  const expensePromise = TimeLog.aggregate([
    ...
    { $match: { 'employeeInfo.employeeType': 'empresa' } },
    { $group: {
        _id:'$employeeInfo._id', totalAmount:{ $sum:'$valorNetoFinal' }, firstDate:{ $first:'$date' },
        fullName:{ $first:'$employeeInfo.fullName' }, cedula:{ $first:'$employeeInfo.idCard' },
        telefono:{ $first:'$employeeInfo.phone' }, direccion:{ $first:'$employeeInfo.address' },
        email:{ $first:'$employeeInfo.email' }
      }
    },
    { $project: {
        _id:0, type:'expense', date:'$firstDate',
        description:{ $concat:["Pago a repartidor ","$fullName"] },
        amount:'$totalAmount', cedula:'$cedula', telefono:'$telefono',
...
const getClientDashboardById = asyncHandler(async (req, res) => {
  ...
  const paymentTotals = await TimeLog.aggregate([{ $match:{ employee:{$in:employeeIds} } }, { $group:{ _id:'$employee', totalAPagar:{ $sum:'$valorNetoFinal' } } }]);
  ...
  const employeesList = client.employees.map(e => ({
    _id: e._id, fullName: e.fullName, idCard: e.idCard, phone: e.phone,
    totalAPagar: totalsMap[e._id.toString()]||0
  }));
  res.json({
    clientProfile:{companyName:client.companyName, defaultHourlyRate:client.defaultHourlyRate, holidayHourlyRate:client.holidayHourlyRate},
    dashboardData:{employeesList, grandTotal}, auxiliaries
  });
});
...
module.exports = {
  getClientDashboardById, getDashboardStats, getCourierDashboardById,
  getEmployeeHistoryForAdmin, getAccountantLedger, registerEmployee,
